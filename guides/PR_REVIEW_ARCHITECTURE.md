...
`aggregate.py` collects one read-only snapshot across the registry using the registered policies; `mode` only labels how far each repository's rollout has progressed. Missing evidence is reported as unavailable with partial totals. Repository-qualified identities prevent identical PR numbers in different repositories from colliding. Combined author totals above five are warnings; there is no cross-repository hard cap.
