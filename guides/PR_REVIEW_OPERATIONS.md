...
3. Inspect the final diff and verification yourself. Post `/self-reviewed` after the bot outcomes — including a skip or a waiver, which count as outcomes — and after this controller has reported on the current head. It is an unedited comment, or the body of your own review, which is one action from the diff you are attesting to; an ordinary review of your own pull request is not an attestation: it attests to everything pushed so far, and its timestamp is checked against that report. `/self-reviewed FULL_HEAD_SHA` names one commit explicitly and needs no such wait. New commits or later bot completion require a new attestation, and an edited comment never counts.
