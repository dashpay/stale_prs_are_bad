...
3. Inspect the final diff and verification yourself. An attestation is the author's own — a colleague posting it on somebody else's pull request counts for nothing. Post `/self-reviewed` after the bot outcomes — including a skip or a waiver, which count as outcomes — and after this controller has reported on the current head. It is an unedited comment, or the body of your own review, which is one action from the diff you are attesting to; an ordinary review of your own pull request is not an attestation: it attests to everything pushed so far, and its timestamp is checked against that report. `/self-reviewed FULL_HEAD_SHA` names one commit explicitly and needs no such wait. New commits require a new attestation. So does any bot report that lands after you attested, because you attested without having read it — and that includes one with no changes request and no thread, since these producers state a blocker in the prose of the report itself ("Add signer support or defer selecting these keys before merging") and "it had nothing to say" cannot be read off the evidence. Answering a finding does not unsay it: the attestation still has to come after it. An edited comment never counts. A pull request opened by an account that runs automation — GitHub's own bots, and the accounts a repository's policy names under `bot_authors` — is asked for none: nobody is there to give one, so the eligible approval it needs anyway stands in. Such an account may not own or review an area, and a policy that names it as one is refused.
