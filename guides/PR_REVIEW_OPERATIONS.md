...
Events reevaluate the affected PR and changes to its author's slot assignments. Only new comments trigger a run, and only those this controller reads: a review bot's, or one containing `/self-reviewed`. The bots edit their own comments as a review progresses and this controller edits its own report, so reacting to edits cost far more runs than it was worth; coverage bots and human discussion cannot change an outcome at all. The filter is on the event payload, so a skipped comment starts no runner. A CodeRabbit completion delivered by editing an existing comment is therefore picked up by the next event or the hourly sweep rather than immediately. Selector-less workflow invocations rotate six PRs at a time and the scheduled repair runs hourly, so a stable 57-PR queue is covered in about ten hours when event signals are missed. Pagination and event bursts can still exhaust quota; errors remain visible. Full local `sync` is an explicit unbounded sweep.
