...
- Create `waiting-bots`, `waiting-self-review`, `ready-for-human`, `too-many-open-prs` and `bot-review-skipped`; the run checks for them once and stops if one is missing rather than creating it. Then opt into writes with repository variable `PR_REVIEW_AUTOMATION_ENABLED=true`.
