...
Events reevaluate the affected PR and changes to its author's slot assignments. Only new comments trigger a run: the bots edit their own comments as a review progresses, and this controller edits its own report, so reacting to edits cost far more runs than it was worth. A CodeRabbit completion delivered by editing an existing comment is therefore picked up by the next event or the hourly sweep rather than immediately. Selector-less workflow invocations rotate three PRs at a time; the scheduled repair runs every 15 minutes. A stable 68-PR queue takes up to six hours to cover if event signals are missed. Pagination and event bursts can still exhaust quota; errors remain visible. Full local `sync` is an explicit unbounded sweep.
