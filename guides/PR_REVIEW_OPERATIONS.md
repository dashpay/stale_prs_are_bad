...
3. Inspect the final diff and verification yourself. Post `/self-reviewed` after the bot outcomes — including a skip or a waiver, which count as outcomes — and after this controller has reported on the current head. It is an unedited comment, or the body of your own review, which is one action from the diff you are attesting to; an ordinary review of your own pull request is not an attestation: it attests to everything pushed so far, and its timestamp is checked against that report. `/self-reviewed FULL_HEAD_SHA` names one commit explicitly and needs no such wait. New commits require a new attestation. So does a bot saying something after you attested — a changes request or a review thread — because you attested without having read it; a bot that finishes with nothing to say does not, since there is nothing in it to read. Answering a finding does not unsay it: the attestation still has to come after it. An edited comment never counts.
